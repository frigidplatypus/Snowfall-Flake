...
with lib.frgd;
let
  cfg = config.frgd.desktop.addons.gtk;
  gdmCfg = config.services.xserver.displayManager.gdm;
in
{
  options.frgd.desktop.addons.gtk = with types; {
    ...
    cursor = {
      ...
      pkg = mkOpt package pkgs.frgd.bibata-cursors "The package to use for the cursor theme.";
    };
    icon = {
