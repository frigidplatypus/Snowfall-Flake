...
in
{
  ...
  config = lib.mkIf cfg.enable (let
    pkg = if cfg.package != null then cfg.package else lib.throw (
    ...
    authKeyPath = if cfg.authKeyFile != null then cfg.authKeyFile
      else if lib.hasAttr "sops" config && lib.hasAttr "secrets" config.sops && lib.hasAttr "tailscale_api_key" config.sops.secrets
      then config.sops.secrets.tailscale_api_key.path
      else null;

    execStart = if authKeyPath != null then
