...
in
{
  options.frgd.cli-apps.taskwarrior = with types; {
    ...
    taskpirate = {
      ...
      hooksDir =
        mkOpt str "~/.local/share/task/hooks"
          "Location (in home) where task hooks will be written";
      defaultDateTime = {
    ...
    in
    {
      ...
      home.file = mkIf cfg.taskpirate.enable {
        ...
        "${hooksDir}/default-time/pirate_add_default_time.py" = mkIf cfg.taskpirate.defaultDateTime.enable {
          text =
            let
              timeParts = builtins.filter builtins.isString (
                builtins.split ":" cfg.taskpirate.defaultDateTime.time
              );
              stripLeadingZeros =
                s:
                if builtins.stringLength s > 1 && builtins.substring 0 1 s == "0" then
                  builtins.substring 1 (builtins.stringLength s - 1) s
                else
                  s;
              hours =
                if builtins.length timeParts >= 1 then
                  lib.strings.toInt (stripLeadingZeros (builtins.elemAt timeParts 0))
                else
                  7;
              minutes =
                if builtins.length timeParts >= 2 then
                  lib.strings.toInt (stripLeadingZeros (builtins.elemAt timeParts 1))
                else
                  0;
              seconds =
                if builtins.length timeParts >= 3 then
                  lib.strings.toInt (stripLeadingZeros (builtins.elemAt timeParts 2))
                else
                  0;
            in
            ''
        ...
        "${hooksDir}/shift-recurrence/pirate_add_shift_recurrence.py" =
          mkIf cfg.taskpirate.shiftRecurrence.enable
            {
        ...
        "${hooksDir}/shift-recurrence/pirate_mod_shift_recurrence.py" =
          mkIf cfg.taskpirate.shiftRecurrence.enable
            {
              source = builtins.fetchurl {
                url = "https://raw.githubusercontent.com/tbabej/task.shift-recurrence/master/pirate_add_shift_recurrence.py";
