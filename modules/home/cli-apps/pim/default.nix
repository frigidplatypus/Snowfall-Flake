...
in
{
      ...
      accountModule =
        types.submodule ({ name, ... }:
          ...
          options = {
            ...
            primaryCollection = mkOpt (nullOr str) null "Name of the primary collection on the remote (defaults to server default).";
            appPasswordSecret = mkOpt str defaultAppPasswordSecret "Name of the SOPS secret containing the app password.";
            ...
            calendarColor = mkOpt str defaultColor
              "Calendar color as rendered in khal.";
            caldavUrl = mkOpt (nullOr str) defaultCaldavUrl "Override the CalDAV URL; defaults to Google if unset.";
            calendarUser = mkOpt (nullOr str) defaultCalendarUser "Override the CalDAV username; defaults to the account email.";
            syncContacts = mkBoolOpt true "Synchronize contacts for this account.";
            carddavUrl = mkOpt (nullOr str) defaultCarddavUrl "Override the CardDAV URL; defaults to Google if unset.";
  ...
  config = mkIf cfg.enable (
    let
      ...
      sopsSecrets =
        listToAttrs (map (account: nameValuePair account.value.appPasswordSecret { }) accountList);
    ...
    defaultCarddavUrl = email: "https://www.googleapis.com/carddav/v1/principals/${email}/lists/default/";
        ...
        listToAttrs (
          concatMap
            (account:
              ...
              in
              optional acc.syncCalendar (nameValuePair name {
                primary = acc.primary;
        ...
        listToAttrs (
          concatMap
            (account:
              ...
              in
              optional acc.syncContacts (nameValuePair name {
                khard = {
                  enable = true;
                };
                vdirsyncer = {
      ...
      primaryCalendarNames =
        lib.filter (n: n != null)
          (mapAttrsToList (
            name: value:
              if value.primary or false then name else null
          ) calendarAccounts);
      defaultCalendarName =
        if hasCalendars then
          if primaryCalendarNames != [ ] then builtins.head primaryCalendarNames
          else builtins.head calendarNames
        else null;
      ...
      (mkIf (cfg.contacts.enable && hasContacts) {
        ...
        programs.khard = {
          ...
          settings = {
            ...
            vcard = {
              private_objects = [ "ANNIVERSARY" "BDAY" ];
